...
export async function handleListSavedQueries(args: any): Promise<HandlerResult> {
    await ensureOrgConfigured();
    let command = `az boards query list`;
    if (args.project) command += ` --project "${args.project}"`;
    if (args.folder) command += ` --path "${args.folder}"`;
    command += ' --output json';

    const { stdout } = await execAsync(command);
    const queries = JSON.parse(stdout);

    const result = queries.value?.map((query: any) => ({
        name: query.name,
        id: query.id,
        path: query.path,
...
export async function handleRunSavedQuery(args: any): Promise<HandlerResult> {
    await ensureOrgConfigured();
    // First get the query definition
    let getCommand = `az boards query show --id "${args.query_id}"`;
    if (args.project) getCommand += ` --project "${args.project}"`;
...
export async function handleCreateSavedQuery(args: any): Promise<HandlerResult> {
    await ensureOrgConfigured();
    // Build the command
...
export async function handleUpdateSavedQuery(args: any): Promise<HandlerResult> {
    await ensureOrgConfigured();
    let command = `az boards query update --id "${args.query_id}"`;
    if (args.name) command += ` --name "${args.name}"`;
    if (args.wiql) command += ` --wiql "${args.wiql}"`;
    if (args.description) command += ` --description "${args.description}"`;
    command += ' --output json';
...
export async function handleDeleteSavedQuery(args: any): Promise<HandlerResult> {
    await ensureOrgConfigured();
